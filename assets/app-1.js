...
(function (PF) {
  ...
  function parseHash() {
    ...
    /* A REAL URL BEATS A FRAGMENT.

       Google does not index what comes after a `#`. Its own URL-structure
       guidance names `example.com/#/potatoes` as the thing not to do, which
       is the shape every study on this site had. So a study that wants to be
       findable gets a file at its own path, and that file says which view it
       is by setting PF_VIEW before this script runs.

       Only consulted when there is no hash at all, so `/#/carry` still wins
       on the index and every existing link keeps working unchanged. */
    if (!h && window.PF_VIEW) return { view: window.PF_VIEW, at: '' };
